#hardcoding necessary angles


### THIS IS A GOLDEN BUILD FOR ARMS ###
### USE THIS SCRIPT ONLY FOR ARMS(BICEPS) EXERCISE ###


##### THIS IS GOLDEN BUILD 5 #####

###10/24/2024 - Updated GOLDEN 4 to readjust some sensor position ###

##### USED FOR 8 NODE IMPLEMENTATION ##


##### GOLDEN BUILD 4 UPDATES ####
#UPDATED CODE TO SUPPORT 8 NODES


##### GOLDEN BUILD 3 UPDATES ####
#FIXED BLUETOOTH NOT CONNECTING TO ALL DEVICES ISSUE
#ADDED SOME CODE RELATED TO YAW 



##### GOLDEN BUILD 2 UPDATES ####
#SCALED FROM ONE SENSOR NODE TO 3 SENSOR NODES
#FEATHER NUMBERS AND CORRESPONDING BODY JOINTS MENTIONED 
#RESCALLING THE VALUES IS DONE BUT NEED TO VERIFY 



#### GOLDEN BUILD 1 UPDATES #####
### BLUETOOTH FEATURES ###
#### This code works fine with multiple nodes (3). Also sends a disconnect signal
## to all nodes if we use ctrl+c to close the terminal
## it may show an error when we use ctrl+c but it works fine
## PLEASE MAKE SURE ALL THE NODES ARE BLINKING SLOWING( INDICATING READY STATE)
## BEFORE YOU ATTEMPT TO RUN THIS SCRIPT AGAIN
## This code also prints the intended devices to be turned on
## This code also DISCOERY RE ATTEMPT code.. it keeps try to discover till
## finds all the intended devices to be turned on.
## DISCOVERY_REATTEMPTS modify this if needed



### UDP FEATURES ###
## Connects to a hardcoded IP of meta quest
## creates an array of 39 angles in json format
## index 21 of the array corresponds to the right hand elbow pitch value

## GOLDEN_ARMS v2
## CHANGED VALUES OF ARMS AND PRINTED NOT CONNECTED VALUES
...
include_list = [RIGHT_HAND_ELBOW,RIGHT_HAND_WRIST,RIGHT_HAND_SHOULDER,LEFT_HAND_SHOULDER,LEFT_HAND_WRIST,LEFT_HAND_ELBOW] #14 as 10
...
def init_position_bicepcurls():
    ...
    generate(0, RIGHT_HAND_WRIST_YAW_INDEX)
    generate(0, RIGHT_HAND_SHOULDER_YAW_INDEX)
    generate(0, RIGHT_HAND_ELBOW_YAW_INDEX)
    generate(0, LEFT_HAND_WRIST_YAW_INDEX)
    generate(0, LEFT_HAND_SHOULDER_YAW_INDEX)
    generate(0, LEFT_HAND_ELBOW_YAW_INDEX)
    ...
    generate(0, RIGHT_HAND_WRIST_ROLL_INDEX)
    generate(0, RIGHT_HAND_SHOULDER_ROLL_INDEX)
    generate(0, RIGHT_HAND_ELBOW_ROLL_INDEX)
    generate(0, LEFT_HAND_WRIST_ROLL_INDEX)
    generate(0, LEFT_HAND_SHOULDER_ROLL_INDEX)
    generate(0, LEFT_HAND_ELBOW_ROLL_INDEX)     
...
def pitch_value_translation_two(pitch_val):
    pitch_data = float(pitch_val)
    scaled_pitch = ((pitch_data + 85) * 180 / 149) - 90 # pitch of right elbow
    return scaled_pitch+5
