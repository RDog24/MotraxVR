...
void setup()
{
  ...
  Bluefruit.setName("Feather 10"); // useful testing with multiple central connections
...
void loop()
{
  ...
  // // float alpha = 0.98;  // Smoothing factor, adjust as needed between 0 and 1
  // // static float xMAG_smooth = xMAG;
  // // static float yMAG_smooth = yMAG;

  // // xMAG_smooth = alpha * xMAG_smooth + (1 - alpha) * xMAG;
  // // yMAG_smooth = alpha * yMAG_smooth + (1 - alpha) * yMAG;

  // // yaw = yawKalmanFilter.updateEstimate(atan2(yMAG_smooth, xMAG_smooth) * RAD_TO_DEG);

  float yaw = yawKalmanFilter.updateEstimate(atan2(-yMAG, xMAG) * RAD_TO_DEG);
